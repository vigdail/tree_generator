...
namespace Color {
constexpr auto Tree = glm::vec3(128.0f, 113.0f, 83.0f) / 255.0f;
...
TreeLayer::TreeLayer() : floor_{MeshFactory::create_plane(100.0f)} {
  load_shader_include("/scene.glsl", "../assets/scene.glsl");
  shader_ = load_shader("../assets/solid.vert", "../assets/solid.frag");
  unlit_shader_ = load_shader("../assets/solid.vert", "../assets/unlit.frag");
}
...
void TreeLayer::on_update(float dt) {
  ...
  shader_->set_uniform(shader_->uniform_location("color"), Color::Tree);
  shader_->set_uniform(shader_->uniform_location("light_dir"), glm::normalize(directional_light_.direction));
  shader_->set_uniform(shader_->uniform_location("light_intensity"), directional_light_.intensity);
  ...
  floor_.bind();
  shader_->set_uniform(shader_->uniform_location("color"), Color::Floor);
  count = static_cast<int32_t>(floor_.get_index_count());
  gl::draw_elements(GL_TRIANGLES, count, GL_UNSIGNED_INT, nullptr);

  if (show_bound_box_) {
    unlit_shader_->use();
    unlit_shader_->set_uniform(unlit_shader_->uniform_location("view_proj"), view_proj);
    unlit_shader_->set_uniform(unlit_shader_->uniform_location("color"), Color::BoundBox);
...
template<typename T>
void gui(const T& instance) {
  rttr::type class_type = rttr::type::get(instance);
  for (const auto& prop: class_type.get_properties()) {
    ...
    if (value.is_type<int>()) {
      ...
      assert(min <= max);
      // TODO: think this need refactoring
      if (min != max) {
    ...
    if (value.is_type<glm::vec3>()) {
      float min = get_metadata_or_default(prop, MetaData::MinValue, 0.0f);
      float max = get_metadata_or_default(prop, MetaData::MaxValue, 0.0f);
      float step = get_metadata_or_default(prop, MetaData::Step, 0.1f);
      auto v = value.get_value<glm::vec3>();
      if (ImGui::DragFloat3(prop.get_name().to_string().c_str(), (float*) &v, step, min, max)) {
        prop.set_value(instance, v);
      }
    }
  }
